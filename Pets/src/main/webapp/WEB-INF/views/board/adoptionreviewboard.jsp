...
<<section class="notice" id="notice-section">
    <div class="page-title" id="page-title">
        <div class="container">
            <h3 class="board-title">입양 후기 게시판</h3>
        </div>
    </div>
                ...
                <tbody>
                    <c:if test="${not empty reviewList}">
                        <c:forEach items="${reviewList}" var="AdoptReviewListDTO">
                            <tr class="board-row">
                                <td class="seq">${AdoptReviewListDTO.seq}</td>
                                <th class="title">
                                    <a href="/pet/reviewdetail.do?seq=${AdoptReviewListDTO.seq}" class="board-title-link">
                                        ${AdoptReviewListDTO.title}
                                    </a>
                                </th>
                                <td class="regdate">${AdoptReviewListDTO.regdate}</td>
                            </tr>
                        </c:forEach>
                    </c:if>
                    <c:if test="${empty reviewList}">
                        <tr>
                            <td colspan="3" class="no-reviews">등록된 리뷰가 없습니다.</td>
                        </tr>
                    </c:if>
                </tbody>
            </table>
            <button class="btn btn-primary rounded-pill d-inline-flex flex-shrink-0 py-2 px-4" 
                    type="submit" onclick="location.href='/pet/reviewadd.do';" class="write-btn">글쓰기</button>

            <!-- 페이징 -->
            <div class="pagination mt-4" id="pagination">
                <c:if test="${currentPage > 1}">
                    <a href="/pet/review.do?page=${currentPage-1}" class="page-link prev">이전</a>
                </c:if>

                <c:forEach var="i" begin="1" end="${totalPages}">
                    <a href="/pet/review.do?page=${i}" 
                ...
                <c:if test="${currentPage < totalPages}">
                    <a href="/pet/review.do?page=${currentPage+1}" class="page-link next">다음</a>
                </c:if>
            </div>
        </div>
    </div>
